...
implements ActionListener, InternalFrameListener, FocusListener {
	...
	public JCheckBox db_logging_checkbox;
	public JCheckBox read_only_checkbox;
	JTextField hostname;
	JTextField username;
	...
	public DB_UserCredentials(DB_Connection db_connection,JDesktopPane desktop) {
		local_db_connection=db_connection;
		local_desktop=desktop;
		initializeClassVariables();
		determineRemoteDefault(hostname.getText());
		doDialog(Main_Panel);
	...
	private void initializeClassVariables() {
		...
		db_logging_checkbox = new JCheckBox("Log DB Actions?");
		read_only_checkbox=new JCheckBox("Read Only?");
	...
	public void doDialog(JPanel parent_panel) {
		...
		p1.add(db_logging_checkbox,class_c);
		p1.add(read_only_checkbox,class_c);
		class_c.anchor=GridBagConstraints.CENTER;
	...
	private void determineRemoteDefault(String t_hostname){
		if ((t_hostname.toLowerCase().contains("localhost"))||
			(t_hostname.toLowerCase().contains("127.0.0.1")))
		{
			db_logging_checkbox.setSelected(true);
			read_only_checkbox.setSelected(true);
		}
			else{
				db_logging_checkbox.setSelected(false);
				read_only_checkbox.setSelected(false);
		}
	}
	...
	private Boolean isFormFilledOut(){
		...
		if ( user_fields.the_hostname.trim().equals("")||
			...
			JOptionPane.showMessageDialog(null, 
					"Please make sure you've entered\n the username and password.", "Form Incomplete",
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (source == okButton) {
			...
			local_frame.dispose();
			local_db_connection.tryConnect();//try to connect with these credentials
		}	
	}
	...
	public void focusLost(FocusEvent e) {
		String evt_str=e.toString();
		if (evt_str.contains("JTextField")){
			//System.out.println("DB_UserCredentials: event was "+evt_str);
			JTextField virtual_tf=(JTextField)e.getComponent();
			determineRemoteDefault(virtual_tf.getText());
		}
	}
